...
async def logUserBot():
    load_dotenv()
    api_id = int(20221457)
    api_hash = "c064f2d573883166675c30b1a12acc8d"
    phone_number = "51990330503"
    ...
    await client.send_message("@spmtrump", f'<b>Bot encendido</b>', parse_mode="HTML")
    spammer_group = int("-4664622852")
    ...
    while True:
        ...
        try:
            await client.send_message("@spmtrump", f"<b>CANTIDAD DE MENSAJES CONSEGUIDOS PARA PUBLICAR</b> <code>{len(messages_list)-1}</code>",parse_mode="HTML")
        except:
            pass
                ...
                if i['group_name'] not in ["Spam","RESPALyaDO🇵🇪BINS PERU","➳𝒀𝑨𝑷𝑬 𝑫𝑬 𝑬𝑺𝑻𝑨𝑭𝑨𝑫𝑶𝑹𝑬𝑺 ✧","QUEMANDO ESTAFADORES","𝐏𝐄𝐑Ú 𝐀𝐘𝐔𝐃𝐀","Referencias Elmer 💸","🎭 CANAL MUNDO STREAMING PERÚ 🇵🇪","TU MARKETPLACE"]:
                    ...
                    for message_spam in messages_list:
                        ...
                        try:
                            ...
                            await client.send_message("@spmtrump", f'<b>Error enviando mensajes a {i["group_id"]}</b> - <code>{i["group_name"]}<code>\nCausa:{error}',parse_mode="HTML")
                            resultado = False
                        if resultado:
                            await client.send_message("@spmtrump", f'<b>Mensaje enviado a {i["group_id"]}</b> - <code>{i["group_name"]}</code>',parse_mode="HTML")  
                        await asyncio.sleep(90)
                        if j==3: break
            await client.send_message("@spmtrump", f'<b>RONDA ACABADA</b>', parse_mode="HTML")
